...
**Cancellation terms are per offer, not per hotel.** The `policies` list sits inside each `roomRates` entry, so refundable and non-refundable rates are separable client side. The `free-cancellation` recipe in this repo does exactly that in about six lines.
